...
// --- Training step (for dataset + trainable memory) ---
...
function maybeHandleMemoryCommand(userText) {
  // very simple pattern: "remember that X" or "remember X"
  ...
  // train model toward answering that fact
  // input: "what did i tell you to remember?"
  // output: the fact itself (toy but effective)
...
function buildContext(userText) {
  // last few turns of chat
  const recent = chatHistory.slice(-4).join(" ");

  // long‑term memory as a simple prefix
